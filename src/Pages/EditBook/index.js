...
class EditBook extends Component {
    ...
    componentDidMount() {
        ...
        const getBookBySlug =  axiosInstance.post('book/getBookBySlug',{'book_slug':match.params.slug});
        
        getBookBySlug.then((response) => {

            const { book , messages, status } = response.data;
            if(status === 1){
                this.setState({ 
                    data : book, 
                    messages:messages ,
    ...
    handleSubmit = (e) => {
        ...
        if(Object.keys(errors).length === 0){
            ...
            try {
                ...
                const response = axiosInstance.post('book/updateBookBySlug', data);
                
                response.then((response) => {
                    
                    const {  messages , book, status } = response.data;
                    ...
                    <div className="container">
                        <div className="middle-content">
                            <Form onSubmit={this.handleSubmit}>
                                ...
                                <FormGroup>
                                    <Button type="submit" className="importCsv btn-primary"  >Update</Button>
                                </FormGroup>
                            </Form>
